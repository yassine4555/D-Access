...
function renderProducts(products) {
  ...
  grid.innerHTML = products.map(p => {
    const currencySymbols = { USD: '$', EUR: '€', GBP: '£', TND: 'DT' };
    ...
    <div class="product-card">
      <div class="product-img-wrap">
        <img src="${escapeAttr(p.imageUrl)}" alt="${escapeAttr(p.name)}" class="product-img"
      ...
      <div class="product-actions">
        <a href="${escapeAttr(p.shopUrl)}" target="_blank" rel="noopener" class="btn btn-sm btn-ghost">🔗 Shop</a>
        <button class="btn btn-sm btn-ghost" onclick="openProductModal('${p._id}')">✏️ Edit</button>
        <button class="btn btn-sm btn-danger" onclick="deleteProduct('${p._id}','${escapeAttr(p.name)}')">Delete</button>
      </div>
    </div>`;
  }).join('');
}
...
function openProductModal(id) {
  ...
  if (id) {
    ...
    document.getElementById('pf-currency').value = p.currency || 'USD';
    document.getElementById('pf-image').value    = p.imageUrl;
    document.getElementById('pf-shop').value     = p.shopUrl;
...
async function submitProductForm(e) {
  ...
  const dto = {
    ...
    shopUrl:     document.getElementById('pf-shop').value.trim(),
    description: document.getElementById('pf-desc').value.trim(),
    isActive:    document.getElementById('pf-active').checked,
  };
  try {
    if (editingProductId) {
      const updated = await api('PATCH', `/admin/products/${editingProductId}`, dto);
      allProducts = allProducts.map(p => p._id === editingProductId ? updated : p);
      toast('Product updated', 'success');
    } else {
      const created = await api('POST', '/admin/products', dto);
